...
// Builds: trees (instanced variants), crooked houses with enter/exit doors, candle props + flicker lights
export function buildWorldExtras({
  ...
  for(let i=0;i<treeCount;i++){
    // scatter mostly outside maze, sometimes inside edges
    let x,z;
    if(rng() < 0.74){
  ...
  for(let i=0;i<houseCount;i++){
    ...
    // place near outer forest, not inside maze (keeps performance & navigation clean)
    ...
    // Add candle light outside near door
...
function makeTreeVariants(){
  ...
  // Variant C: dead spire (more Silent Hill-ish)
  ...
  return [
    ...
    { trunkGeo: trunkGeoC, crownGeo: crownGeoC, trunkMat, crownMat: crownMatC, trunkY:1.1, crownY:2.4 },
  ];
}

function makeCrookedHouse({ rng, id=0, level=1 }){
